...
try:
    ...
    EWZ_RKI.rename(columns={
        'attributes.RS': 'AGS',
        'attributes.EWZ': 'EWZ',
        'attributes.last_update': 'Datenstand'}, inplace=True)
    datenstand = pd.to_datetime(EWZ_RKI['Datenstand'].iloc[0], format='%d.%m.%Y, %H:%M Uhr')
    ...
    LK.drop([
        ...
        'attributes.AnzTodesfallM'], axis=1, inplace=True)
    LK.rename(columns={
        ...
        'attributes.AnzFall100kW': 'AnzFall100kW'}, inplace=True)
    ...
    calcBV.drop([
        ...
        'AnzFallW',
        'AnzFall100kM',
        'AnzFall100kW'], inplace=True, axis=1)
    ...
    maskBV = (
        (BV['GueltigAb'] <= datenstand.date().strftime('%Y-%m-%d')) &
